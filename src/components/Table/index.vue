    ...
    <div class="table card">
        ...
        <el-table :data="tableData" style="width: 100%;" v-loading="loading">
            ...
            <el-table-column fixed="right" label="操作" width="180"
                v-if="['View', 'Del', 'Edit'].some((value: any) => btns.includes(value))">
                <template #default="scope">
...
import { ref, toRefs, reactive, Ref, ReactiveEffect } from 'vue'
import { DataType, ColumnType } from '../SearchForm/type'
import { GoodsListRes } from '@/api/goods/type'
...
import {OptionType} from '@/components/SearchForm/type'
...
// ----------------------表格-----------------------------
const tableData = ref<GoodsListRes[]>([] as GoodsListRes[])
const getTypeLabel = (arr:OptionType[],e: string):string | unknown => {
    if (e === null || e === undefined || e === '') return ''
    if (arr === null || arr === undefined || !arr.length) return ''
    return arr.find(item => item.value === e)?.label
}
const delClick = (e: number):void => {
...
const searchColumns:ColumnType[] = props.columns.filter((item) => item.search?.tag)
const dialogVisible = ref<boolean>(false)
const title = ref<string>('新增出库')
...
const viewClick = (e: GoodsListRes):void => {
    title.value = '详情'
    disabled.value = true
    setForm(e)
    dialogVisible.value = true
...
const editClick = (e: GoodsListRes, i: number):void => {
    title.value = '编辑'
    disabled.value = false
    editIndex.value = i
    setForm(e)
